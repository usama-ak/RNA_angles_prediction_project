#include "x3dna.h"
enum { TOR_XXX =
        0, TOR_ALPHA, TOR_BETA, TOR_GAMMA, TOR_DELTA, TOR_EPSILON, TOR_ZETA, TOR_CHI,
    TOR_ETA, TOR_THETA, TOR_ETA1, TOR_THETA1, TOR_ETA2, TOR_THETA2,
    TOR_V0, TOR_V1, TOR_V2, TOR_V3, TOR_V4, TOR_TM, TOR_PHASE
};
enum { NT_NUM =
        0, NT_P, NT_O5, NT_C5, NT_C4, NT_C3, NT_O3, NT_C2, NT_C1, NT_O4, NT_N, NT_C,
    O3P_LKG
};
...
{
    fprintf(fp, "START TOSION ANGLE\n");
}
static void output_footer_bb_torsion(FILE * fp)
{
    fprintf(fp, "END TOSION ANGLE\n");
}
...
void atom_list(long ds, long num_bp, long **pair_num, long **seidx, long *RY,
               ...
               long *ResSeq, char **Miscs, long **phos, long **c6_c8, long **sugar,
               long **chi)
{
...
void output_nt_pseudo_torsion(long num_residue, char **nt_info, long **nt_list,
                       double **nt_torsion, double **ss_Zp_Dp)
{
    char separator[] = ",";
    char *bstr = "NA", *fmt = "%4.1f";
    char ostr[BUF512];
    long i, j;
    printf("%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n","rank", separator, "base", separator,
        "eta", separator,"theta", separator, "eta'", separator,
           "theta'" , separator, "eta\"", separator, "theta\"");
    for (i = 1; i <= num_residue; i++) {
        if (nt_list[i][NT_NUM] < 3)
            continue;
         if (i ==1)
         {sprintf(ostr, "%s", separator);}
        sprintf(ostr, "%ld%s%s", i, separator, nt_info[i]);
        for (j = TOR_ETA; j <= TOR_THETA2; j++)
        {
            strcat(ostr, separator);
                       ...
                       double **nt_torsion, double **ss_Zp_Dp)
{
    ...
    printf("%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n", "rank", separator,
     "base", separator, "chi", separator, "A/S",separator,"alpha",separator,"beta",
     separator,"gamma",separator, "delta",separator,"epsilon",separator,"zeta",
     separator,"e-z",separator,"BI/BII");
    for (i = 1; i <= num_residue; i++) {
        if (nt_list[i][NT_NUM] < 3)
            continue;
        sprintf(ostr, "%ld%s%s%s", i,separator, nt_info[i], separator);
                      ...
                      long **chi, double **xyz, double **nt_bb_torsion, FILE * fp)
{
    ...
    print_sep(fp, '*', 76);
    if (ds == 2)
        print_duplex_torsions(num_bp, pair_num, bp_seq, chi_angle, nt_bb_torsion, bstr,
                              fmt, fp);
    else
        print_ss_torsions(num_bp, pair_num, bp_seq, chi_angle, nt_bb_torsion, bstr, fmt,
                          fp);
    print_sep(fp, '*', 76);
...
void lambda_d3(long num_bp, char **bp_seq, long **chi, long **c6_c8, double **xyz,
               FILE * fp)
{
...
{
    sprintf(bp_sym, "%c%c%c", (bp_type == 2) ? '-' : '*', (bp_type > 0) ? '-' : '*',
            zdir);
}
                ...
                long *WC_info, long *str_type, long irna, long **o3p_brk)
{
    ...
    free_pdb(NUM_RESIDUE_ATOMS, NULL, sAtomName, sResName, sChainID, sResSeq, sxyz,
             sMiscs);
                 ...
                 FILE * fp)
{
    ...
    if (ishel)
        fprintf(fp,
                "    step       X-disp    Y-disp   h-Rise"
                "     Incl.       Tip   h-Twist\n");
    else
        fprintf(fp,
                "    step       Shift     Slide      Rise"
                "      Tilt      Roll     Twist\n");
...
static void double_helix(long num_bp, char **bp_seq, double **step_par, double **heli_par,
                         ...
                         long istep, long bz, long *str_type, long **pair_num,
                         char **nt_info)
{
              ...
              long *bz_junction, long *z_step)
{
    ...
    if (dot(x1, x2) > 0.0 && dot(z1, z2) > 0.0 && dot(y1_osx, y2) > 0.0
        && dot(dorg, z1) < 0.0 && dot(dorg, z2) < 0.0) {
        for (i = 1; i <= 3; i++) {
            r1[i][1] = -r1[i][1];
            r1[i][3] = -r1[i][3];
            r2[i][1] = -r2[i][1];
...
void print_ref(char **bp_seq, long num_item, long ich, double *org, double *orien,
               FILE * fp)
{
    ...
    for (i = 1; i <= num_bp - 1; i++) {
        ...
        for (j = 1; j <= tnum_res1 + tnum_res2; j++) {
            ...
            change_xyz(0, &mst_org[ioffset3], mst, seidx[jr][2] - seidx[jr][1] + 1,
                       xyz_residue);
            pdb_record(seidx[jr][1], seidx[jr][2], &inum, 1, AtomName, ResName, ChainID,
              ...
              double **xyz, long *WC_info, long *bphlx, long abi, long **chi, FILE * fp)
{
    ...
    print_xyzP(parallel, nbpm1, bp_seq, phos, mst_orien, mst_org, xyz, fchek, str, aveS,
               p_offset);
    sprintf(str,
            "xyz coordinates of O1P atoms w.r.t. the middle helix frame"
            " of each dimer\n\n");
    ...
    print_xyzP(parallel, nbpm1, bp_seq, phos, mst_orien, mst_org, xyz, fchek, str, aveS,
               p_offset);
    sprintf(str,
            "xyz coordinates of O2P atoms w.r.t. the middle helix frame"
            " of each dimer\n\n");
    strcat(str, "    step      xIH     yIH     zIH     xIIH    yIIH    zIIH\n");
    print_xyzP(parallel, nbpm1, bp_seq, phos, mst_orienH, mst_orgH, xyz, fchek,
               str, aveH, p_offset);
    ...
    print_xyzP(parallel, nbpm1, bp_seq, phos, mst_orien, mst_org, xyz, fchek, str, aveS,
               p_offset);
    sprintf(str,
            "xyz coordinates of P atoms w.r.t. the middle helix frame"
            " of each dimer\n\n");
    strcat(str, "    step      xIH     yIH     zIH     xIIH    yIIH    zIIH\n");
    print_xyzP(parallel, nbpm1, bp_seq, phos, mst_orienH, mst_orgH, xyz, fchek,
               str, aveH, p_offset);
    ...
    for (i = 1; i <= nbpm1; i++) {
        ...
        if (ip && !bphlx[i]) {
            ...
            if (WC_info[i] && WC_info[i + 1] &&
                ...
                dval_in_range(aveH[i][2], 1.5, 10.0) &&
                dval_in_range(aveH[i][3], -3.0, 9.0)) {
                if (aveS[i][3] >= 1.5)
                    strABT[i] = 1;
                    ...
                    double **org, long *bphlx, FILE * fp)
{
    ...
    if (ds == 1) {
        print_sep(fp, '*', 76);
        fprintf(fp,
                "Position (Px, Py, Pz) and local helical axis vector"
                " (Hx, Hy, Hz)\n\n");
        fprintf(fp,
                "     step       Px        Py        Pz"
                "        Hx        Hy        Hz\n");
               ...
               double *zave)
{
    ...
    if (ds == 2) {
        ...
        for (j = 1; j <= 3; j++)
            zave[j] +=
                (d > 0.0) ? orien[ds][ioffset9 + 6 + j] : -orien[ds][ioffset9 + 6 + j];
        d = dot(&orien[1][ioffset9 + 6], &orien[ds][ioffset9 + 15]);
        for (j = 1; j <= 3; j++)
            zave[j] +=
                (d > 0.0) ? orien[ds][ioffset9 + 15 + j] : -orien[ds][ioffset9 + 15 + j];
    }
    vec_norm(zave);
}
               ...
               double *bp_org, FILE * fp)
{
    ...
    for (i = 1; i <= ds; i++) {
        for (j = 1; j <= num_bp; j++) {
            ...
            cross(o1, o2, ppos);
            bp_seq[0][j] = (i == 2
                            && dot(ppos, &orien[1][ioffset + 6]) < 0.0) ? '-' : '+';
            if (dot(z, ppos) < 0)
                negate_xyz(z);
            vec_orth(z, y);
                    ...
                    double *bp_orien, double *bp_org, FILE * fp)
{
    ...
    fprintf(fp,
            "    step       X-disp    Y-disp   h-Rise     Incl.       Tip"
            "   h-Twist\n");
    for (i = 1; i <= num_bp; i++) {
        fprintf(fp, " %4ld %c%c%c ", i, bp_seq[1][i], bp_seq[0][i], bp_seq[2][i]);
        for (j = 1; j <= 6; j++)
...
void out_cehs(long num_bp, char **bp_seq, long *bphlx, double **orien, double **org,
              FILE * fp)
{
