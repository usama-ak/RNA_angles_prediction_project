                                    ...
                                    long *WC_info, long *bphlx, double **xyz, FILE * fp)
{
    ...
    fprintf(fp,
            "This structure contains %ld non-Watson-Crick (with leading *) base pair(s)\n",
            num);
    NNy = simple & SIMPLE_BP_LONG_AXIS_RN9_YN1;
    simple_bp_pars(orien, org, NNy, c6_c8, chi, num_bp, bp_seq, WC_info, xyz, fp);
    simple_step_heli_pars(orien, org, chi, num_bp, bp_seq, WC_info, bphlx, xyz, FALSE,
                          fp);
    if (simple & SIMPLE_STEP_HELICAL_PARS)
        simple_step_heli_pars(orien, org, chi, num_bp, bp_seq, WC_info, bphlx, xyz, TRUE,
                              fp);
}
...
static void process_str(char *inpfile, struct_args * args)
{
    ...
    base_overlap(ds, num_bp, num, num_residue, pair_num, RY, bp_seq, seidx, AtomName, xyz,
                 idx, orien, org, fp);
    nbpm1 = num_bp - 1;
    ...
    if (bbexist) {
        backbone_torsion(ds, num_bp, pair_num, bp_seq, sugar, chi, xyz, nt_bb_torsion,
                         fp);
...
static void calculate_torsions(char *pdbfile, int is_pseudo)
{
    ...
    if (is_pseudo){
        output_nt_pseudo_torsion(num_residue, nt_info, nt_list, nt_torsion, ss_Zp_Dp);
    } else{
        output_nt_torsion(num_residue, nt_info, nt_list, nt_torsion, ss_Zp_Dp);
    }
    free_pdb(num, NULL, AtomName, ResName, ChainID, ResSeq, xyz, Miscs);
    free_lmatrix(seidx, 1, num_residue, 1, 2);
    free_lvector(RY, 1, num_residue);
...
static void analyze_cmdline(int argc, char *argv[], struct_args * args)
{
    ...
    calculate_torsions(argv[i], is_pseudo) ;
