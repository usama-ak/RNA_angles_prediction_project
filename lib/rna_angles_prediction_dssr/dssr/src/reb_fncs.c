                 ...
                 long xdir, char **bp_seq, double **step_par, char *BDIR, char *outfile)
{
    ...
    for (i = 1; i <= num_bp; i++) {
        ...
        snum =
            read_pdb(spdb, NULL, sAtomName, sResName, sChainID, sResSeq, sxyz, NULL, 1,
                     "*");
        if (xdir)
            for (j = 1; j <= snum; j++) {
                sxyz[j][1] = -sxyz[j][1];
    ...
    free_pdb(num_max_per_residue, NULL, sAtomName, sResName, sChainID, sResSeq, sxyz,
             NULL);
                ...
                double **xyz1, double **xyz2, char ap)
{
    ...
    *num1 =
        read_pdb(fname1, NULL, AtomName1, ResName, ChainID, ResSeq, xyz1, NULL, 1, "*");
    *num2 =
        read_pdb(fname2, NULL, AtomName2, ResName, ChainID, ResSeq, xyz2, NULL, 1, "*");
    if (xdir) {
        for (i = 1; i <= *num1; i++) {
            xyz1[i][1] = -xyz1[i][1];
            xyz1[i][3] = -xyz1[i][3];
                ...
                double **step_par, char *BDIR, char *outfile)
{
    ...
    for (i = 1; i <= num_bp; i++) {
        if (is_single)
            ...
            sprintf(bpi, "%c%c", toupper((int) bp_seq[i][1]),
                    toupper((int) bp_seq[i][2]));
